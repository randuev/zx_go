...
func parseCLI() *cliFlags {
	var (
		...
		recordAudio         = flag.String("record-audio", "", "Capture the mixed audio output to this WAV path from startup (the exact stream sent to the device). Diagnostic for clicks/pops that only appear live — the file is ground truth for what we emit. Works with NO sound card (headless included): when the audio device is unavailable a silent mixer is attached and pumped once per emulated frame, so the WAV still carries the exact beeper+AY+DAC mix.")
